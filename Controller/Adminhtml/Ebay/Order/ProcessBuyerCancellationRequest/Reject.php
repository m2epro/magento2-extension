...
class Reject extends \Ess\M2ePro\Controller\Adminhtml\Ebay\Order
{
    ...
    public function execute()
    {
        ...
        if ($success = $result->getSuccess()) {
            $this->messageManager->addSuccessMessage(
                __(
                    "Declining of the order cancellation request(s) for %count order(s) is being processed.",
                    [
                        'count' => $success,
                    ]
                )
        ...
        if ($errors = $result->getErrors()) {
            $this->messageManager->addErrorMessage(
                __(
                    "Cancellation request(s) for %count order(s) were not declined.",
                    [
                        'count' => $errors,
                    ]
                )
        ...
        if ($notAllowed = $result->getNotAllowed()) {
            $this->messageManager->addWarningMessage(
                __(
                    "Cancellation request(s) for %count order(s) is not available to be declined.",
                    [
                        'count' => $notAllowed,
                    ]
                )
