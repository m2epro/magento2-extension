...
class Decline extends \Ess\M2ePro\Controller\Adminhtml\Ebay\Order
{
    ...
    public function execute()
    {
        ...
        if ($success = $result->getSuccess()) {
            $this->messageManager->addSuccessMessage(
                (string)__(
                    'Declining of the order Return request(s) for %count order(s) is being processed.',
                    [
                        'count' => $success,
                    ]
                )
        ...
        if ($errors = $result->getErrors()) {
            $this->messageManager->addErrorMessage(
                (string)__(
                    'Return request(s) for %count order(s) were not declined.',
                    [
                        'count' => $errors,
                    ]
                )
        ...
        if ($notAllowed = $result->getNotAllowed()) {
            $this->messageManager->addWarningMessage(
                (string)__(
                    'Return request(s) for %count order(s) is not available to be declined.',
                    [
                        'count' => $notAllowed,
                    ]
                )
