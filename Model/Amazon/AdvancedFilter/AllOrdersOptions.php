...
class AllOrdersOptions
{
    ...
    private function getBaseSelect(): \Magento\Framework\DB\Select
    {
        ...
        $select->joinInner(
            ['amazon_order' => $this->amazonOrderResource->getMainTable()],
            'amazon_order.order_id = order_id',
            []
        );

        $select->where('orders.component_mode = ?', \Ess\M2ePro\Helper\Component\Amazon::NICK);
