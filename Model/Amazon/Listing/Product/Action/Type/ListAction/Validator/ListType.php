...
class ListType extends \Ess\M2ePro\Model\Amazon\Listing\Product\Action\Type\Validator
{
    ...
    public function __construct(
        ...
        parent::__construct($activeRecordFactory, $amazonFactory, $helperFactory, $modelFactory, $messageFactory);
    ...
    public function validate()
    {
        ...
        if ($this->getVariationManager()->isIndividualType() && !$this->validateComplexMagentoProductTypes()) {
            $this->addMessage(
                ...
                self::ERROR_REQUIRE_MANUAL_ASIN_SEARCH
            );
    ...
    private function recognizeBySearchGeneralId()
    {
        ...
        if (empty($amazonData)) {
            $this->addMessage(
                ...
                self::ERROR_GENERAL_ID_NOT_FOUND
            );
        ...
        if (count($amazonData) > 1) {
            $this->addMessage(
                ...
                self::ERROR_MULTIPLE_PRODUCTS_FOUND_GENERAL_ID
            );
        ...
        if (!empty($amazonData['parentage']) && $amazonData['parentage'] == 'parent') {
            if (!$this->getVariationManager()->isRelationParentType()) {
                $this->addMessage(
                    ...
                    self::ERROR_PARENT_FOUND_CHILD_EXPECTED_GENERAL_ID
                );
            ...
            if (!empty($amazonData['bad_parent'])) {
                $this->addMessage(
                    ...
                    self::ERROR_AMAZON_RESTRICTIONS_BY_GENERAL_ID
                );
            ...
            if (count($magentoAttributes) != count($amazonDataAttributes)) {
                $this->addMessage(
                    ...
                    self::ERROR_VARIATION_ATTRIBUTES_MISMATCH_GENERAL_ID
                );
        ...
        if ($this->getVariationManager()->isRelationParentType()) {
            $this->addMessage(
                ...
                self::ERROR_CHILD_FOUND_PARENT_EXPECTED
            );
    ...
    private function recognizeBySearchWorldwideId()
    {
        ...
        if ($worldwideId->hasUnresolvedType()) {
            ...
            $this->addMessage(
                ...
                self::ERROR_INVALID_WORLDWIDE_ID
            );
        ...
        if (empty($amazonData)) {
            ...
            $this->addMessage(
                ...
                self::ERROR_WORLDWIDE_ID_NOT_FOUND
            );
        ...
        if (count($amazonData) > 1) {
            $this->addMessage(
                ...
                self::ERROR_MULTIPLE_PRODUCTS_FOUND_WORLDWIDE_ID
            );
        ...
        if (
            ...
            $this->addMessage(
                ...
                self::ERROR_PARENT_FOUND_CHILD_EXPECTED_WORLDWIDE_ID
            );
    ...
    private function recognizeWorldwideId()
    {
        ...
        if ($parentAmazonListingProduct->getGeneralId() != $parentGeneralId) {
            $this->addMessage(
                ...
                self::ERROR_CHILD_RELATED_TO_ANOTHER_PARENT
            );
        ...
        if (!isset($parentChannelVariations[$generalId])) {
            $this->addMessage(
                ...
                self:: ERROR_NO_CHILD_WITH_VARIATION_MATCH
            );
    ...
    private function validateNewProduct()
    {
        ...
        if (empty($productTypeTemplate)) {
            $this->addMessage(
                ...
                self::ERROR_PRODUCT_TYPE_MISSING
            );
        ...
        if ($this->getVariationManager()->isRelationMode()) {
            ...
            if (empty($channelTheme)) {
                $this->addMessage(
                    ...
                    self::ERROR_VARIATION_THEME_MISSING
                );
        ...
        if (!empty($amazonData)) {
            $this->addMessage(
                ...
                self::ERROR_CREATE_IDENTIFIER_FAILED
            );
