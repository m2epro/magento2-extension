...
{
    private const MAX_LENGTH_OPTION_VALUE = 50;
    ...
    protected function prepareOptionTitles($optionTitles)
    {
        foreach ($optionTitles as &$optionTitle) {
            $optionTitle = self::formatOptionValue((string)$optionTitle);
        }
    ...
    public static function formatOptionValue(string $input): string
    {
        return trim(
            \Ess\M2ePro\Helper\Data::reduceWordsInString(
                $input,
                self::MAX_LENGTH_OPTION_VALUE
            )
        );
    }
}
