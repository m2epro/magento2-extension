...
namespace Ess\M2ePro\Model\Ebay\Dashboard\Sales;
...
class Calculator implements \Ess\M2ePro\Model\Dashboard\Sales\CalculatorInterface
{
    ...
    public function getAmountPointSet(DateRange $dateRange, bool $isHourlyInterval): PointSet
    {
        return $this->getPointSet('SUM(paid_amount)', $dateRange, $isHourlyInterval);
    }

    public function getQuantityPointSet(DateRange $dateRange, bool $isHourlyInterval): PointSet
    {
    ...
    private function getPointSet(string $valueColumn, DateRange $dateRange, bool $isHourlyInterval): PointSet
    {
        ...
        $select->where(
            sprintf(
                'payment_status = %s OR payment_status = %s',
