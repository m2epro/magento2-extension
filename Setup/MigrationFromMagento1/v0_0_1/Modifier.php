 ...
 *
 * Migration from 6.5.0.9 to 1.2.1
 */

namespace Ess\M2ePro\Setup\MigrationFromMagento1\v0_0_1;
...
class Modifier implements \Ess\M2ePro\Setup\MigrationFromMagento1\IModifierInterface
{
    ...
    private function migrateArchivedEntity()
    {
        $archivedEntity = $this->getConnection()->newTable(
            ...
            ->addColumn(
                'data',
                \Magento\Framework\DB\Ddl\Table::TYPE_TEXT,
                InstallSchema::LONG_COLUMN_SIZE,
                ['nullable' => false]
            )
            ->addColumn(
                'create_date',
    ...
    private function migrateOther()
    {
        $this->getConnection()->delete(
            $this->getFullTableName('module_config'),
            ['`group` REGEXP \'^\/component\/(ebay|amazon|buy){1}\/$\' AND `key` = \'allowed\'']
        );

        $this->getConnection()->delete(
            $this->getFullTableName('module_config'),
            ['`group` = \'/view/common/component/\'']
        );

        $this->getConnection()->delete(
            $this->getFullTableName('module_config'),
            ['`group` = \'/view/common/autocomplete/\'']
        );
